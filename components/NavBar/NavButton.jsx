...
const NavButton = ({ children, Icon, title, href }) => {
    const {scroll} = useScroll();
    const {size} = useScreenSize();

    return (
        <Link href={href} title={title} className="aspect-auto sm:aspect-square hover:scale-105 w-full h-fit sm:h-full sm:w-fit" style={{ padding: interpolate(scroll, [0, 150], [3, 1]) + "px" }} to="">
            <div style={{ borderRadius: size === State.sm ? 0 : interpolate(scroll, [0, 150], [50, 0]) + "%" }} 
